...
from app.domain.legal_evidence.models import ABA_AI_COMPETENCE_RULES, STATE_AI_CLE_REQUIREMENTS
...
class FRE707Service:
    ...
    def register(self, conn: Any, *, organization_id: str,
                 ...
                 hdr_id: str | None = None, hash_sha256: str = "",
                 created_by: str) -> dict[str, Any]:
        evidence_id = str(uuid.uuid4())
        # FRE 702 checklist score (0-4)
        fre702_score = sum([
            methodology_disclosed, reliable_principles,
            principles_applied, opinion_not_speculative,
        ])
        self._repo.create(
            conn, evidence_id=evidence_id, organization_id=organization_id,
            case_ref=case_ref, court=court, jurisdiction=jurisdiction,
            document_ref=document_ref, document_type=document_type,
            ai_system_name=ai_system_name, ai_system_version=ai_system_version,
            ai_provider=ai_provider, ai_model_id=ai_model_id,
            ...
            validation_method=validation_method, error_rate_known=error_rate_known,
            error_rate_value=error_rate_value, peer_reviewed=peer_reviewed,
            ...
            hdr_id=hdr_id, hash_sha256=hash_sha256,
    ...
    def update_admissibility(self, conn: Any, evidence_id: str, *,
        ...
        self._repo.update_admissibility(
            conn, evidence_id, admissibility_opinion=admissibility_opinion,
            court_ruling=court_ruling, conditions=conditions,
        )
        return {"evidence_id": evidence_id, "admissibility_opinion": admissibility_opinion}
...
class CitationVerificationService:
    ...
    def verify(self, conn: Any, *, organization_id: str,
               ...
               bar_complaint_risk: str = "none") -> dict[str, Any]:
        citation_id = str(uuid.uuid4())
        self._repo.create(
            conn, citation_id=citation_id, organization_id=organization_id,
            document_ref=document_ref, case_ref=case_ref,
            citation_text=citation_text, citation_type=citation_type,
            cited_court=cited_court, cited_year=cited_year,
            reporter=reporter, volume=volume, page_start=page_start, url=url,
            verified_by=verified_by, verification_method=verification_method,
            verification_db=verification_db, verification_date=verification_date,
            citation_exists=citation_exists, proposition_accurate=proposition_accurate,
            quote_accurate=quote_accurate, case_still_good_law=case_still_good_law,
            is_hallucination=is_hallucination, hallucination_type=hallucination_type,
        ...
        accuracy_score = round(sum(1 for f in accuracy_flags if f) / len(accuracy_flags) * 100)
        return {
...
class HallucinationService:
    ...
    def report(self, conn: Any, *, organization_id: str,
               citation_id: str | None = None, document_ref: str = "",
               ...
               workflow_updated: bool = False, created_by: str) -> dict[str, Any]:
        incident_id = str(uuid.uuid4())
        self._repo.create(
            conn, incident_id=incident_id, organization_id=organization_id,
            citation_id=citation_id, document_ref=document_ref, case_ref=case_ref,
            incident_type=incident_type, ai_system=ai_system, ai_model=ai_model,
            original_output=original_output, correct_info=correct_info,
            severity=severity, filed_with_court=filed_with_court,
            court_sanction=court_sanction, financial_impact=financial_impact,
            client_notified=client_notified, bar_reported=bar_reported,
            root_cause=root_cause, prevention_measure=prevention_measure,
            workflow_updated=workflow_updated, created_by=created_by,
        )
    ...
    def resolve(self, conn: Any, incident_id: str, *, resolved_by: str) -> dict[str, Any]:
...
class AICompetenceService:
    ...
    def issue_certificate(self, conn: Any, *, organization_id: str,
                          attorney_id: str, attorney_name: str = "",
                          bar_number: str = "", jurisdiction: str = "",
                          training_provider: str = "", training_course: str = "",
                          cle_credits_earned: float = 0, training_date: str = "",
        ...
        self._repo.create(
            conn, cert_id=cert_id, organization_id=organization_id,
            attorney_id=attorney_id, attorney_name=attorney_name,
            bar_number=bar_number, jurisdiction=jurisdiction,
            training_provider=training_provider, training_course=training_course,
            cle_credits_earned=cle_credits_earned, training_date=training_date,
            training_topics=training_topics, ai_systems_covered=ai_systems_covered,
            ...
            certificate_number=cert_number, issued_by=issued_by or "Heillon Legal",
