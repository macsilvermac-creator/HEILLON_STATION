    ...
    def register(
        ...
        *,
        ...
        if ai_code_applicable and not transparency_notice_published:
            warnings.append(
                "UK AI Code of Practice: transparency notice required for AI systems."
            )
        if profiling_used and not profiling_basis:
            warnings.append(
                "UK GDPR Art. 22: legal basis required for automated profiling decisions."
            )
        ...
        self._repo.create(
            ...
            profiling_basis=profiling_basis,
            ...
            dpo_name=dpo_name,
            uk_rep_appointed=uk_rep_appointed,
            uk_rep_name=uk_rep_name,
    ...
    def register(
        ...
        *,
        ...
        if aida_applicable and high_impact_system and not impact_assessment_done:
            warnings.append(
                "Bill C-27 AIDA: impact assessment required for high-impact AI systems."
            )
        ...
        if law_25_quebec and q25_pia_required and not q25_pia_done:
            warnings.append(
                "Quebec Law 25: Privacy Impact Assessment (PIA) required before deployment."
            )
        ...
        self._repo.create(
            ...
            ai_system_ref=ai_system_ref,
            ai_system_name=ai_system_name,
            provincial_law=provincial_law,
            law_25_quebec=law_25_quebec,
            consent_obtained=consent_obtained,
            consent_form=consent_form,
            implied_consent_basis=implied_consent_basis,
            withdrawal_mechanism=withdrawal_mechanism,
            aida_applicable=aida_applicable,
            high_impact_system=high_impact_system,
            ...
            q25_pia_required=q25_pia_required,
            q25_pia_done=q25_pia_done,
        ...
        *,
        ...
        if agentic_ai_applicable:
            if not agentic_human_oversight:
                warnings.append(
                    "PDPC Agentic AI Framework: human oversight mechanism required."
                )
            if not agentic_disclosure:
                warnings.append(
                    "PDPC Agentic AI Framework: AI agent disclosure to users required."
                )
        if cbdt_countries and not (
            cbdt_contractual_clauses
            or cbdt_binding_corporate_rules
            or cbdt_adequacy_applicable
        ):
            warnings.append(
                "PDPA Part IX: transfer mechanism required for cross-border data transfers."
            )
        ...
        self._repo.create(
            ...
            ai_system_ref=ai_system_ref,
            ai_system_name=ai_system_name,
            pdpa_dpo_designated=pdpa_dpo_designated,
            pdpa_dpo_name=pdpa_dpo_name,
        ...
        if agentic_ai_applicable:
            agentic_score = sum(
                [
                    agentic_human_oversight,
                    agentic_disclosure,
                    bool(agentic_consent_scope),
                    agentic_data_minimised,
        ...
        *,
        ...
        if adm_used and adm_meaningful_impact:
            if not adm_explanation_available:
                warnings.append(
                    "Privacy Act (proposed 2026): right to explanation for meaningful AI decisions."
                )
            if not adm_human_review_available:
                warnings.append(
                    "Privacy Act (proposed 2026): right to human review for meaningful AI decisions."
                )
        if ndb_scheme_applicable and not breach_assessment_process:
            warnings.append(
                "NDB Scheme: breach assessment and OAIC notification process required."
            )
        ...
        self._repo.create(
            ...
            adm_used=adm_used,
            adm_description=adm_description,
        ...
        app_score = round(
            sum(
                [
                    app1_privacy_policy,
                    app5_collection_notice,
                    app6_primary_purpose_only,
                    bool(app11_security_measures),
                    bool(app12_access_process),
