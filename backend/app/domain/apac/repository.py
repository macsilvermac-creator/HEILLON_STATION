...
class UKGDPRRepository:
    def create(self, conn: Any, *, record_id: str, organization_id: str,
        ...
        conn.execute(
            ...
            (
                record_id, organization_id, ai_system_ref, ai_system_name,
                ico_reference, int(ico_registered), int(data_protection_fee_paid),
                lawful_basis, legitimate_interests_assessment,
                int(ai_code_applicable), int(transparency_notice_published),
                int(human_review_available), int(profiling_used), profiling_basis,
                right_access_process, right_erasure_process,
                right_portability_process, right_object_ai,
                int(dpo_required), dpo_name, int(uk_rep_appointed), uk_rep_name,
                eu_transfer_mechanism,
                json.dumps(international_transfers or {}),
                int(dpia_conducted), dpia_ref,
...
class CanadaPrivacyRepository:
    def create(self, conn: Any, *, record_id: str, organization_id: str,
        ...
        conn.execute(
            ...
            (
                record_id, organization_id, ai_system_ref, ai_system_name,
                provincial_law, int(law_25_quebec),
                int(consent_obtained), consent_form, implied_consent_basis, withdrawal_mechanism,
                int(aida_applicable), int(high_impact_system),
                json.dumps(high_impact_categories or []),
                int(impact_assessment_done), impact_assessment_ref,
                json.dumps(mitigation_measures or []), incident_reporting_process,
                q25_privacy_officer, int(q25_privacy_policy_published),
                int(q25_pia_required), int(q25_pia_done),
                int(q25_72h_breach_report), int(q25_portability_enabled),
                int(breach_reported_to_opc), opc_file_number,
                "active", created_by, now, now,
            ),
        )
...
class SingaporePDPARepository:
    def create(self, conn: Any, *, record_id: str, organization_id: str,
        ...
        conn.execute(
            ...
            (
                record_id, organization_id, ai_system_ref, ai_system_name,
                int(pdpa_dpo_designated), pdpa_dpo_name, int(pdpa_dpo_registered),
                int(data_protection_policy_published), int(do_not_call_compliant),
                int(consent_purpose_specific), int(notification_given),
                int(deemed_consent_applied),
                int(agentic_ai_applicable), int(agentic_human_oversight),
                agentic_oversight_desc, int(agentic_disclosure), agentic_disclosure_text,
                agentic_consent_scope, int(agentic_data_minimised), agentic_incident_plan,
                int(pdpc_model_governance_aligned), int(explainability_implemented),
                int(bias_testing_done),
                json.dumps(cbdt_countries or []),
                int(cbdt_contractual_clauses), int(cbdt_binding_corporate_rules),
...
class AustraliaPrivacyRepository:
    def create(self, conn: Any, *, record_id: str, organization_id: str,
        ...
        conn.execute(
            ...
            (
                record_id, organization_id, ai_system_ref, ai_system_name,
                annual_turnover_aud, int(health_service_provider), int(acts_covered),
                int(app1_privacy_policy), int(app5_collection_notice),
                int(app6_primary_purpose_only),
                app11_security_measures, app12_access_process, app13_correction_process,
                int(adm_used), adm_description, int(adm_explanation_available),
                int(adm_human_review_available), int(adm_opt_out_available),
                int(adm_meaningful_impact),
                int(ndb_scheme_applicable), breach_assessment_process,
                oaic_notification_process, oaic_complaint_process,
