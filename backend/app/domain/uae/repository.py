        ...
        *,
        ...
        conn.execute(
            ...
            (
                ...
                legal_basis,
                int(sensitive_data_processing), int(biometric_data),
                int(health_data), int(children_data), int(guardian_consent_obtained),
                int(cross_border_transfer), transfer_destination_country, transfer_safeguards,
                int(difc_jurisdiction), int(adgm_jurisdiction),
                consent_text, ip_address, language,
    ...
    def create(
        ...
        *,
        ...
        conn.execute(
            ...
            (
                gov_id, organization_id, ai_system_name, ai_system_version, ai_system_purpose,
                int(ethics_human_centered), int(ethics_fairness), int(ethics_transparency),
                int(ethics_safety_reliability), int(ethics_privacy_security),
                int(ethics_accountability), int(ethics_sustainability),
                int(ai_seal_applied), ai_seal_reference, ai_seal_category,
                sector,
                int(difc_compliant), difc_registration_ref, "2020",
                int(adgm_compliant), adgm_registration_ref,
                jurisdiction_ae, risk_level, risk_assessment_notes,
        ...
        *,
        ...
        conn.execute(
            ...
            (
                sig_id, organization_id, document_ref, document_hash, document_title,
                signatory_name, signatory_email, signatory_emirates_id, signatory_mobile,
                int(uae_pass_verified), uae_pass_identity_level, uae_pass_session_ref,
                trust_service_provider, trust_service_level, qtsp_country,
                signature_format, signature_level,
                signed_at or now, tsa_timestamp, tsa_provider, hdr_id,
                "valid", now,
            ),
        )
