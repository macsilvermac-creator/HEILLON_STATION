...
class ColoradoSB26189Service:
    ...
    def register(self, conn: Any, *, organization_id: str,
        ...
        warnings: list[str] = []
        is_exempt = small_business_exempt or open_source_exempt or national_security_exempt
        if not is_exempt:
            if not disclosure_provided:
                warnings.append("SB 26-189 Art. 8: Consumer disclosure required before consequential AI decision.")
            if not explanation_available:
                warnings.append("SB 26-189 Art. 9: Right to explanation must be available.")
            if not data_correction_available:
                warnings.append("SB 26-189 Art. 10: Right to data correction must be available.")
            if not human_review_available:
                warnings.append("SB 26-189 Art. 11: Right to human review must be available.")
        ...
        self._repo.create(
            ...
            disclosure_provided=disclosure_provided, disclosure_timing=disclosure_timing,
        ...
        rights_score = sum([
            disclosure_provided, explanation_available,
            data_correction_available, human_review_available,
        ])
        return {
            "record_id": record_id,
            "warnings": warnings,
...
class CCPAADMTService:
    ...
    def register(self, conn: Any, *, organization_id: str,
        ...
        if not pre_use_notice_provided:
            warnings.append("CCPA ADMT §7085: Pre-use notice required before deployment.")
        ...
        if risk_assessment_required and not risk_assessment_done:
            warnings.append("CCPA ADMT Art. 22: Risk assessment required before deployment.")

        self._repo.create(
            conn, admt_id=admt_id, organization_id=organization_id,
            ai_system_ref=ai_system_ref, ai_system_name=ai_system_name,
            admt_purpose=admt_purpose, significant_decisions=significant_decisions,
            personal_data_used=personal_data_used, california_consumers=california_consumers,
            ...
            opt_out_available=opt_out_available, opt_out_mechanism=opt_out_mechanism,
            opt_out_response_days=opt_out_response_days,
            global_opt_out_honored=global_opt_out_honored,
            access_to_admt_logic=access_to_admt_logic, access_process=access_process,
            human_review_available=human_review_available,
            human_review_process=human_review_process,
        ...
        rights_score = sum([
            pre_use_notice_provided, opt_out_available,
            access_to_admt_logic, human_review_available,
        ])
        return {
            "admt_id": admt_id,
            "warnings": warnings,
...
class MalpracticeInsuranceService:
    ...
    def register(self, conn: Any, *, organization_id: str,
        ...
        self._repo.create(
            conn, insurance_id=insurance_id, organization_id=organization_id,
            law_firm_name=law_firm_name, bar_jurisdiction=bar_jurisdiction,
            insurer_name=insurer_name, policy_number=policy_number,
            policy_start=policy_start, policy_end=policy_end,
            coverage_limit_usd=coverage_limit_usd, current_premium_usd=current_premium_usd,
            ai_tools_used=ai_tools_used, ai_tools_list=ai_tools_list,
...
class HeilonComplianceScoreService:
    ...
    def compute(self, conn: Any, *, organization_id: str,
                ai_system_ref: str, ai_system_name: str = "",
                component_scores: dict | None = None,
                evidence_bundle: dict | None = None,
                computed_by: str = "system") -> dict[str, Any]:
        ...
        self._repo.create(
            conn, score_id=score_id, organization_id=organization_id,
            ai_system_ref=ai_system_ref, ai_system_name=ai_system_name,
