        ...
        *,
        ...
        conn.execute(
            ...
            (
                ...
                ai_system_name,
                ai_system_version,
                consequential_decision_type,
                consumers_affected_count,
                int(disclosure_provided),
                disclosure_timing,
                disclosure_method,
                int(explanation_available),
                explanation_process,
                explanation_response_days,
                int(data_correction_available),
                data_correction_process,
                int(human_review_available),
                human_review_process,
                human_review_response_days,
                int(opt_out_available),
                json.dumps(opt_out_categories or []),
                cure_period_days,
                int(ag_notice_received),
                ag_notice_date,
                int(cure_completed),
                cure_completion_date,
                int(small_business_exempt),
                int(open_source_exempt),
        ...
        *,
        ...
        conn.execute(
            ...
            (
                ...
                ai_system_name,
                admt_purpose,
                int(significant_decisions),
                int(personal_data_used),
                int(california_consumers),
                int(pre_use_notice_provided),
                pre_use_notice_content,
                notice_delivery_method,
                int(opt_out_available),
                opt_out_mechanism,
                opt_out_response_days,
                int(global_opt_out_honored),
                int(access_to_admt_logic),
                access_process,
                int(human_review_available),
                human_review_process,
                human_review_timing,
                int(risk_assessment_required),
                int(risk_assessment_done),
                risk_assessment_ref,
...
class MalpracticeInsuranceRepository:
        ...
        *,
        ...
        conn.execute(
            ...
            (
                ...
                current_premium_usd,
                int(ai_tools_used),
                json.dumps(ai_tools_list or []),
                int(ai_outputs_filed_in_court),
                int(citation_verification_process),
                hallucination_incidents_12mo,
                ...
                ai_related_claims_count,
                ai_related_claims_usd,
                "active",
                created_by,
                now,
                now,
    ...
    def get(self, conn: Any, insurance_id: str) -> dict[str, Any] | None:
        row = conn.execute(
            "SELECT * FROM malpractice_insurance_records WHERE insurance_id=?",
            (insurance_id,),
        ).fetchone()
        return dict(row) if row else None
...
class HeilonScoreRepository:
    def create(
        ...
        *,
        ...
        total_score: int = 0,
        certification_tier: str = "unrated",
        ...
        conn.execute(
            ...
            (
                ...
                scores.get("score_lgpd", 0),
                scores.get("score_gdpr_eu", 0),
                scores.get("score_gdpr_uk", 0),
                scores.get("score_ccpa", 0),
                scores.get("score_colorado", 0),
                scores.get("score_pdpl_uae", 0),
                scores.get("score_pdpa_sg", 0),
                scores.get("score_privacy_au", 0),
                scores.get("score_pipeda_ca", 0),
                scores.get("score_iso42001", 0),
                scores.get("score_iso27001", 0),
                scores.get("score_nist_rmf", 0),
                scores.get("score_euai_act", 0),
