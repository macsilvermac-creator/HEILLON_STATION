...
class ISO42001Repository:
    def create(self, conn: Any, *, aims_id: str, organization_id: str,
               ...
               # Clause 10
               c10_nonconformities: list | None = None, c10_corrective_actions: list | None = None,
               c10_continual_improvement_plan: str = "",
               # Annex A
               annex_a2: bool = False, annex_a3: bool = False, annex_a4: bool = False,
               annex_a5: bool = False, annex_a6: bool = False, annex_a7: bool = False,
        ...
        conn.execute(
            ...
            (
                aims_id, organization_id, ai_system_ref, ai_system_name, ai_system_version,
                c4_internal_issues, c4_external_issues,
                json.dumps(c4_interested_parties or []),
                c4_aims_scope, int(c4_ai_policy_defined),
                int(c5_top_mgmt_commitment), c5_ai_policy_text,
                int(c5_roles_defined), int(c5_dpo_appointed),
                int(c6_risks_assessed), int(c6_opportunities_noted),
                json.dumps(c6_objectives_set or []), json.dumps(c6_action_plans or []),
                int(c7_resources_allocated), int(c7_competence_verified),
                int(c7_awareness_training), int(c7_documentation_maintained),
                int(c8_operational_controls), c8_ai_system_lifecycle,
                int(c8_data_quality_assured), int(c8_human_oversight_active),
                c8_incident_response_plan,
                json.dumps(c9_monitoring_metrics or {}),
                int(c9_internal_audit_done), int(c9_mgmt_review_done),
                c9_last_audit_date,
                json.dumps(c10_nonconformities or []), json.dumps(c10_corrective_actions or []),
                c10_continual_improvement_plan,
                int(annex_a2), int(annex_a3), int(annex_a4), int(annex_a5),
                int(annex_a6), int(annex_a7), int(annex_a8), int(annex_a9),
                certification_body, certification_status,
    ...
    def update_certification(self, conn: Any, aims_id: str, *,
                             ...
                             certificate_expires_at: str | None = None) -> None:
        now = _now()
        conn.execute(
            """UPDATE iso42001_aims_records
               ...
               WHERE aims_id=?""",
            (certification_status, conformity_score, certification_body,
             certificate_number, certificate_expires_at, now, aims_id),
        )

    def add_control_log(self, conn: Any, *, log_id: str, aims_id: str,
                        control_ref: str, control_name: str = "",
                        evidence: str = "", status: str = "planned",
...
class FRIARepository:
    def create(self, conn: Any, *, fria_id: str, organization_id: str,
               ai_system_ref: str, ai_system_name: str = "",
        ...
        conn.execute(
            ...
            (
                fria_id, organization_id, ai_system_ref, ai_system_name,
                intended_purpose, foreseeable_misuse, geographic_scope, population_affected,
                int(right_dignity), int(right_privacy), int(right_nondiscrimination),
                int(right_fair_trial), int(right_presumption), int(right_labour),
                int(right_education), int(right_property), other_rights,
                impact_severity, impact_likelihood, impact_description,
                int(vulnerable_groups_affected), vulnerable_groups_desc,
                ...
                residual_risk_level, int(deployment_approved), deployment_conditions,
                review_frequency,
                assessor_id, assessor_name, int(dpo_consulted), int(legal_reviewed),
                status, 1, now, now,
            ),
        )
    ...
    def approve(self, conn: Any, fria_id: str, *, approved_by: str,
                deployment_conditions: str = "") -> None:
        now = _now()
        conn.execute(
