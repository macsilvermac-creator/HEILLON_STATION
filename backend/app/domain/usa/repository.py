        ...
        *,
        ...
        conn.execute(
            ...
            (
                ...
                developer_name, deployer_name, risk_tier, high_risk_category,
                consequential_decision_desc,
                int(impact_assessment_done), impact_assessment_date, impact_assessment_ref,
                int(bias_audit_done), bias_audit_date, bias_audit_provider,
                consumer_notification_text, opt_out_mechanism, int(appeal_process_available),
                monitoring_plan, "[]", "active", created_by, now, now,
            ),
        )
        ...
        *,
        ...
        conn.execute(
            ...
            (
                ...
                consent_type,
                int(sensitive_data_consent), int(automated_decision_consent),
                int(sale_of_personal_info_consent), int(sharing_for_cross_context),
                consent_text, ip_address, "active", expires_at, now, now,
            ),
        )
        ...
        *,
        ...
        conn.execute(
            ...
            (
                log_id, organization_id, matter_ref, attorney_id, attorney_name,
                ai_tool_name, ai_tool_version, ai_tool_provider,
                int(rule_11_competence), int(rule_16_confidentiality),
                int(rule_34_fairness), int(rule_53_supervision),
                int(client_disclosure_made),
                state_bar, state_specific_rule_ref, state_specific_notes,
                int(output_reviewed), review_notes, now,
            ),
        )
    ...
    def create(
        ...
        *,
        ...
        conn.execute(
            ...
            (
                rmf_id, organization_id, ai_system_ref, ai_system_name, ai_system_version,
                int(govern_policies_defined), int(govern_roles_assigned),
                int(govern_risk_tolerance_set), int(govern_training_completed), govern_notes,
                map_intended_use, int(map_context_established),
                json.dumps(map_risks_identified or []),
                int(map_stakeholders_consulted), map_notes,
                int(measure_metrics_defined), int(measure_testing_completed),
                int(measure_bias_evaluated),
                measure_performance_score, measure_trustworthiness, measure_notes,
                json.dumps(manage_risk_responses or []),
                json.dumps(manage_residual_risks or []),
                manage_monitoring_plan, manage_incident_plan, manage_notes,
                profile_tier, "active", created_by, now, now,
            ),
        )
        ...
        *,
        ...
        conn.execute(
            ...
            (
                audit_id, organization_id, sig_id, event_type, event_sequence,
                actor_id, actor_name, actor_email, actor_ip,
                document_ref, document_hash, event_hash, raw, now,
            ),
        )
        return event_hash
